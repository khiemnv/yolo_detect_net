...
namespace Yolov
{
    public static class PlotCV
    {
        ...
        public static (double, double, OpenCvSharp.Point[]) GetContour(IMask mask)
        {
            var vinoMask = (VinoMask)mask;

            Cv2.FindContours(vinoMask.mask,
                out Point[][] contours,
            ...
            var me = Cv2.MinAreaRect(contours[conIdx]);
            var angle = me.Angle;
            ...
            var area = Cv2.ContourArea(contours[conIdx]);
            return (angle, area, contours[conIdx]);
        }
